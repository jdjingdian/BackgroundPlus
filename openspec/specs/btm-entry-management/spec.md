...
系统 SHALL 提供以“侧栏功能分组 + 主区域列表”为结构的 BTM 条目展示。
侧栏必须至少包含“登录时打开”和“允许在后台”两个入口；主区域列表必须仅展示当前入口对应的条目集合，并包含每行的 icon、主标题、副标题、开关状态与行级 `→` 操作入口。当 helper 已安装时，数据源必须来自真实系统 dump 结果而非固定占位数据。

#### Scenario: 进入登录时打开分组

- **WHEN** 用户在侧栏选择“登录时打开”
- **THEN** 系统必须只展示被归类为登录时打开的条目，并隐藏允许在后台条目

#### Scenario: 进入允许在后台分组

- **WHEN** 用户在侧栏选择“允许在后台”
- **THEN** 系统必须只展示被归类为允许在后台的条目，并隐藏登录时打开条目
...
- **WHEN** 系统完成解析与分类
- **THEN** 系统必须提示“分类不完整”并继续展示可确定分类的条目
